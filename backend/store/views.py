...
from store.serializers import (
    CancelledOrderSerializer,
    CartSerializer,
    CartOrderItemSerializer,
    CouponUsersSerializer,
    ProductSerializer,
    TagSerializer,
    CategorySerializer,
...
class CouponApiView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if coupon:
            ...
            if order_items:
                for i in order_items:
                    ...
                    if not coupon in i.coupon.all():
                        ...
                        order.save()
                        return Response(
                            {"message": "Coupon Activated"}, status=status.HTTP_200_OK
                        )
                    else:
                        return Response(
            ...
            return Response(
                {"message": "Order Item Does Not Exists"}, status=status.HTTP_200_OK
            )
        else:
            return Response(
                {"message": "Coupon Does Not Exists"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def create(self, request, *args, **kwargs):
            ...
            if response.status_code == 200:
                ...
                if paypal_payment_status == "COMPLETED":
                    if order.payment_status == "processing":
                        ...
                        return Response(
                            {"message": "Already Paid"}, status=status.HTTP_201_CREATED
                        )
        ...
        if session_id != "null":
            ...
            if session.payment_status == "paid":
                ...
                return Response(
                    {"message": "unpaid!"}, status=status.HTTP_402_PAYMENT_REQUIRED
                )
            elif session.payment_status == "canceled":
                return Response(
                    {"message": "cancelled!"}, status=status.HTTP_403_FORBIDDEN
                )
            else:
                return Response(
